...
async function inseeSiretGet(siret) {
    ...
    if (!res.err) return { key: siret, val: res.val.etablissement, err: null }
    if (res.err) return { key: siret, val: null, err: res.err }
}
...
async function inseeSirenGet(siren) {
    ...
    if (!res.err) return { key: siren, val: res.val.uniteLegale, err: null }
    if (res.err) return { key: siren, val: null, err: res.err }
}
...
async function inseeNafGet(code, norm) {
    ...
    if (!res.err) return { key: code, val: res.val, err: null }
    if (res.err) return { ley: code, val: null, err: res.err }
}
