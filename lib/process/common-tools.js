...
async function getNaf(code, norm) {
    let res = null
    try {
        res = await inseeGet.naf(code, norm)
    } catch (err) {
        console.log('naf code', code, norm, err)
    }
    return res
}


async function getCat(code, type) {
    let res = null
    try {
        res = await inseeGet.cat(code, type)
    } catch (err) {
        console.log('cat code', code, type, err)
    }
    return res
}
